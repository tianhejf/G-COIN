...
// Copyright (c) 2013 The G-coin developers
...
uint256 hashGenesisBlock("0x0000059cbecf2b6e7af1a97068e0f7600aba7d369260a0b2800a12be81428eaf");
static const unsigned int timeGenesisBlock = 1454426962;
...
const string strMessageMagic = "G-coin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees, unsigned int nBits)
{
	...
	if (nHeight >= 1 && nHeight < 58) 
    {
        nSubsidy = 0 * COIN;
    }
	else if (nHeight == 58 )
    {
        nSubsidy = 10800000000 * COIN;
    }
else if (nHeight > 58 )
    {
        nSubsidy = 0 * COIN;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
         const char* pszTimestamp = "GXCOIN in already databases we GXCOIN from USA with reindexing.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0xc72496dd69ab8fb4bd34de0c46eaf7a7c7523382cba1fed625d03af324b1dd6d"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("G-coinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("G-coinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("G-coinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("G-coin-miner");
    ...
    try { loop {
        ...
        printf("Running G-coinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
